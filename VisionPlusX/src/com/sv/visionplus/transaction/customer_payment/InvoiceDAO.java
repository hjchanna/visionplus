...
public class InvoiceDAO {
    ...
    int updateStatus(Connection connection, TInvoice invoice) {
        ...
        invoice.setAmount(searchInvoice.getAmount());
        invoice.setCustomer(searchInvoice.getCustomer());
        invoice.setFactory(searchInvoice.getFactory());
        invoice.setInvoiceDate(new Date());
        invoice.setIsDelete(searchInvoice.getIsDelete());
        try {
            InvoiceId = Query.executeUpdate(connection, invoice, "index_no=?", invoice.getIndexNo());
        } catch (SQLException ex) {
            Logger.getLogger(InvoiceDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
        return InvoiceId;
